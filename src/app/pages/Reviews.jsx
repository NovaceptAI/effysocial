...
export default function Reviews() {
  ...
  return (
    <div>
      <PageHeader title="Reviews" subtitle={`${pending} awaiting response`} actions={<Button variant="secondary" onClick={() => setRequesting(true)}>Request reviews</Button>} />
      {requesting && <RequestReviews onClose={() => setRequesting(false)} />}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[300px_1fr] gap-4">
        ...
        <Card className="p-5 h-max">
          <div className="text-center mb-4">
            ...
            <Stars n={total ? Math.round(avg) : 0} />
            <div className="text-xs text-ink-faint mt-1">{total} review{total === 1 ? '' : 's'}</div>
          </div>
          <div className="space-y-1.5">
            {dist.map((d) => (
              <div key={d.star} className="flex items-center gap-2 text-xs">
                ...
                <div className="flex-1 h-2 rounded-full bg-surface2 overflow-hidden"><div className="h-full bg-warning rounded-full" style={{ width: `${total ? (d.count / total) * 100 : 0}%` }} /></div>
            ...
            <Card key={r.id} className="p-4">
              <div className="flex items-start justify-between gap-3">
                <div>
                  ...
                  <div className="text-xs text-ink-faint mt-0.5 capitalize">{r.source === 'direct' ? 'private feedback' : r.source} · <span className={r.sentiment === 'negative' ? 'text-error' : r.sentiment === 'positive' ? 'text-success' : ''}>{r.sentiment}</span></div>
                </div>
                ...
                <div className="mt-3">
                  {expanded === r.id ? (
                    <div className="p-3 rounded-lg bg-coral-soft/50 border border-coral-soft">
                      ...
                      <p className="text-sm text-ink-soft">{r.reply || 'No suggestion for this one — write your own reply.'}</p>
                      <p className="text-[0.7rem] text-ink-faint mt-1.5">
                        {r.source === 'direct'
                          ? 'This came privately through your review link — reply to the customer directly.'
                          : `Post your reply on ${r.source[0].toUpperCase() + r.source.slice(1)} yourself; replying from here needs Business Profile access, which isn't set up yet.`}
                      </p>
                      <div className="flex gap-2 mt-2">
                        <Button size="sm" disabled={respond.isPending} onClick={() => { respond.mutate(r.id); setExpanded(null); }}>
                          <Send className="w-3.5 h-3.5" /> {respond.isPending ? 'Saving…' : 'Mark as replied'}
                        </Button>
