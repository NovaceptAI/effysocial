...
import { Search, Plus, Sparkles, Bell, ChevronDown, Check, LogOut, AlertTriangle, Info, ShieldAlert, CheckSquare, Menu } from 'lucide-react';
...
export default function TopBar({ onOpenPalette, onOpenAssistant, onOpenNav }) {
  ...
  return (
    <header className="h-16 shrink-0 sticky top-0 z-20 bg-canvas border-b border-hair flex items-center gap-2 sm:gap-3 px-3 sm:px-6">
      ...
      <div className="relative shrink-0">
        <button
          ...
          className="flex items-center gap-2.5 pl-1.5 pr-2 sm:pr-2.5 py-1.5 rounded-lg bg-transparent hover:bg-surface2 transition"
        >
        ...
        <Dropdown open={wsOpen} onClose={() => setWsOpen(false)} className="left-0 w-72">
          ...
          {workspaces.map((w) => (
            <button
              ...
              className="w-full flex items-center gap-2.5 px-3 py-2 bg-transparent hover:bg-surface2 transition text-left"
            >
      ...
      {/* Command / search — icon-only, opens the command palette (⌘K still works) */}
      <button onClick={onOpenPalette} title="Search (⌘K)" className="grid place-items-center w-9 h-9 rounded-lg bg-transparent hover:bg-surface2 transition text-ink-soft shrink-0">
